...
void gpSceneUpdate(gpScene *scene, float gameTime) {
	...
	for (size_t i = 0; i < _currentNumBodies; i++) {
		...
		if (!body) continue;
		memcpy(body->lastFrameOverlaps, body->overlaps,
			   sizeof(gpOverlap) * body->numOverlappingBodies);
		body->lastFrameNumOverlappingBodies = body->numOverlappingBodies;
		body->numOverlappingBodies = 0;
		if (!body->gravityEnabled) {
			// We should still check if we are overlapping with other things,
			// even if we aren't checking for movement
	...
	for (size_t i = 0; i < _currentNumBoxColliders; i++) {
		...
		if (!box) continue;
		memcpy(box->lastFrameOverlaps, box->overlaps,
			   sizeof(gpOverlap) * box->numOverlappingBodies);
...
static void CheckForNonStaticBoxOverlaps(gpBoxCollider *box) {
	if (!_isEnabled || !box->enabled) return;
	...
	for (size_t i = 0; i < _currentNumBodies; i++) {
		...
		if (overlapBody->bodyType == box->bodyType || !overlapBody->enabled)
			continue;
		// Check to see if this overlap already happened (this happens inside of
		// static body)
		...
		gpBB overlapArea;
		int intersect = gpIntersectRect(
			&box->boundingBox, &overlapBody->boundingBox, &overlapArea);
		if (intersect) {
			int direction = gpCalculateIntersectionDirection(&overlapArea,
															 &box->boundingBox);
	...
	for (size_t i = 0; i < _currentNumStaticBodies; i++) {
		...
		// For now, skip if it is the same body type for testing
		if (overlapBody->bodyType == box->bodyType) continue;
		// Check to see if this overlap already happened (this happens inside of
		// static body)
		bool newOverlap = true;
		for (size_t j = 0; j < box->numOverlappingBodies; j++) {
...
static void CheckForNonStaticOverlaps(gpBody *body) {
	if (!_isEnabled || !body->enabled) return;
	// Check for non static bodies
	for (size_t i = 0; i < _currentNumBodies; i++) {
		gpBody *overlapBody = _currentBodies[i];
		if (overlapBody == body || !overlapBody->enabled) continue;
		// Check to see if this overlap already happened (this happens inside of
		// static body)
		bool newOverlap = true;
		...
		gpBB overlapArea;
		int intersect = gpIntersectRect(
			&body->boundingBox, &overlapBody->boundingBox, &overlapArea);
		if (intersect) {
			int direction = gpCalculateIntersectionDirection(
				&overlapArea, &body->boundingBox);
...
static void ApplyYVelocity(gpBody *body, float gameTime) {
	if (!body->yGravityEnabled) return;
	...
	double stepSize =
		(int)initialYStep != 0 ? initialYStep > 0 ? 1 : -1 : initialYStep;
	...
	while (shouldStep && _isEnabled) {
		...
		for (size_t i = 0; i < _currentNumStaticBodies; i++) {
			...
			int intersect =
				gpIntersectBoxBox(&body->boundingBox, &staticBody->boundingBox);
			if (intersect) {
				if (body->staticCollisionEnabled) {
					gpResolveOverlapY(&body->boundingBox,
									  &staticBody->boundingBox);
					shouldStep = 0;
					if (direction == gpOverlapDown) body->thisFrameOnGround = 1;
				}
		...
		// then send out notify for subscribers with info of collision bounding
		// box and body num
		if (!shouldStep) {
			// If we are set to be blocked by the other body,
			// then set should step to 0, and revert body back to initial
			// body->boundingBox.y -= stepSize;
			if (body->staticCollisionEnabled) body->velocity.y = 0.0;
			continue;
		}
		iterYStep -= stepSize;
...
static void ApplyXVelocity(gpBody *body, float gameTime) {
	if (!body->xGravityEnabled) return;
	double initialXStep = body->velocity.x * gameTime;
	double iterXStep = initialXStep;
	double stepSize =
		(int)initialXStep != 0 ? initialXStep > 0 ? 1 : -1 : initialXStep;
	int shouldStep = stepSize != 0 ? 1 : 0;
	...
	while (shouldStep && _isEnabled) {
		...
		// For body in bodies, if collides,
		// then send out notify for subscribers with info of collision bounding
		// box and body num If it is a blocking body, then we should set
		// shouldStep to False
		for (size_t i = 0; i < _currentNumStaticBodies; i++) {
			gpBody *staticBody = _currentStaticBodies[i];
			int intersect =
			...
			if (intersect) {
				if (body->staticCollisionEnabled) {
					gpResolveOverlapX(&body->boundingBox,
									  &staticBody->boundingBox);
					shouldStep = 0;
				}
				gpBodyAddOverlap(body, staticBody, direction);
...
gpScene *gpInitScene(void) {
	...
	_currentBodies = calloc(_currentCapacityBodies,
							_currentCapacityBodies * sizeof(gpBody *));
	_currentStaticBodies =
		calloc(_currentCapacityStaticBodies,
			   _currentCapacityBoxColliders * sizeof(gpBody *));
	_currentBoxColliders =
		calloc(_currentCapacityBoxColliders,
			   _currentCapacityBoxColliders * sizeof(gpBoxCollider *));
	return scene;
}
...
int gpSceneAddBody(gpBody *body) {
	if (_currentNumBodies > _currentCapacityBodies / 2) {
		_currentBodies = realloc(_currentBodies,
								 _currentCapacityBodies * 2 * sizeof(gpBody *));
		if (_currentBodies == NULL) {
			fprintf(stderr,
					"Couldn't reallocate to increase body size, what the");
		}
...
int gpSceneAddStaticBody(gpBody *body) {
	if (_currentNumStaticBodies > _currentCapacityStaticBodies / 2) {
		_currentStaticBodies =
			realloc(_currentStaticBodies,
					_currentCapacityStaticBodies * 2 * sizeof(gpBody *));
		if (_currentStaticBodies == NULL) {
			fprintf(
				stderr,
				"Couldn't reallocate to increase static body size, what the");
		}
...
void gpSceneSetEnabled(int isEnabled) { _isEnabled = isEnabled; }
...
int gpSceneAddBoxCollider(gpBoxCollider *boxCollider) {
	if (_currentNumBoxColliders > _currentCapacityBoxColliders / 2) {
		_currentBoxColliders =
			realloc(_currentStaticBodies,
					_currentCapacityBoxColliders * 2 * sizeof(gpBoxCollider *));
