...
static void expandArraySize(geContentTypes type)
{
    ...
    if (info->Count + 1 >= info->Size / 2)
    {
        ...
        _loadedContent[type] = realloc(_loadedContent[type], newSize * sizeof(geContent *));
...
int geAddContentTypeCreateFunc(geContentTypes type, ContentTypeCreateFunc func)
{
...
int geAddContent(geContentTypes type, void *data)
{
    ...
    content->Type = type;
    if (!_newContentFunctions[type])
        LogCritical("Content type is not available, please register this function properly!");
